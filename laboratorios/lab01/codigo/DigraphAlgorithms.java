...
public class DigraphAlgorithms {
     ...
     */
    public static int numSucesores(Graph gd) {
        int verticeMax = 0;
        int mayorS = 0;
     ...
     */
    public static ArrayList DFS(Graph gd, int ini, int fin) {
     ...
     */
    public static boolean DFS(Graph gd, int ini, int fin, ArrayList<Integer> visitados, boolean[] flags) {
        visitados.add(ini);
        flags[ini] = true;
        if (ini == fin) {
            return true;
     ...
     */
    public static boolean tieneCiclos (Graph gd){
     ...
     */
    public static boolean tieneCiclos(Graph gd, int ini, int fin, boolean [] flags, boolean [] tiene) {        
        flags[ini] = true;
        tiene [0] = false;
        if (ini == fin) {
            return true;
     ...
     */
    public static ArrayList BFS(Graph gd, int ini, int fin) {
        Queue<Integer> cola = new LinkedList<Integer>();
        ArrayList<Integer> visitados = new ArrayList<>(gd.size());
        boolean[] flags = new boolean[gd.size()];
        BFS(gd, ini, fin, visitados, flags, cola);
        return visitados;
    }
     ...
     * @param visitados Arreglo que se retornara con los vertices en orden de visitados.
     * @param flags Arreglo de booleanos para marcar vertices como visitados.
     * @param cola Cola en la que se almecenaran todos los hijos de un determinado 
     * vertice (necesario para BFS).
     * @return true, si encontro un camino desde el vertice inicial hasta el 
     * vertice final, false si ocurrio lo contrario.
     */
    public static boolean BFS(Graph gd, int ini, int fin,
        ArrayList<Integer> visitados, boolean[] flags, Queue<Integer> cola) {
        visitados.add(ini);
        flags[ini] = true;        
        if (ini == fin) {
            return true;
        ...
        while (!cola.isEmpty()){
                if (!flags[cola.peek()] && BFS(gd,cola.poll(), fin, visitados, flags, cola)) {
                    return true;
               }
        }
        return false;
